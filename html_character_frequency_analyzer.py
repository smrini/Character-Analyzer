...
print("Welcome to the HTML Analyzer!")
print("This program fetches a webpage and analyzes the frequency of different characters in its HTML code.")
...
def analyze_url(url):
    ...
    # Initialize the counters with all possible characters
    letters = Counter({ch: 0 for ch in string.ascii_lowercase})
    numbers = Counter({ch: 0 for ch in string.digits})

    # Update the counters with the characters from the HTML code
    letters.update(ch for ch in html_code.lower() if ch in string.ascii_lowercase)
    numbers.update(ch for ch in html_code if ch.isdigit())
    symbols = Counter(ch for ch in html_code if ch in string.punctuation)
